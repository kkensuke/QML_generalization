...
\begin{itemize}
    ...
    \item $\hat{R}_N(f) \defeq \frac1N \sum_{i=1}^{N} \calL(y_i, f(\bs{x}_i))$: empirical risk of a hypothesis $f\in\calF$ over the training set $\calS$
    ...
    \item $\calF\bullet\calS \defeq \set{(f(\bs{x}_1),\ldots,f(\bs{x}_N))}[f \in \calF, (\bs{x}_i,y_i)\in\calS] \subset [0, C]^N$: prediction evaluated over the training set $\calS$
    \item $\calG\bullet\calS \defeq \set{(\calL(y_1,f(\bs{x}_1)), \ldots, \calL(y_N,f(\bs{x}_N)))}[f \in \calF, (\bs{x}_i,y_i)\in\calS] \subset [0, LC]^N$: loss function evaluated over the training set $\calS$
\end{itemize}
