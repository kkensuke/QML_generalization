...
\begin{itemize}
    ...
    \item $\hat{R}_N(f_{\bth}) \defeq \frac1N \sum_{i=1}^{N} \calL(y_i, f_{\bth}(\bs{x}_i))$: empirical risk of a hypothesis $f_{\bth}\in\calF$ over the training set $\calS$
    ...
    \item $\calF\bullet\calS \defeq \set{(f_{\bth}(\bs{x}_1),\ldots,f_{\bth}(\bs{x}_N))}[f_{\bth}\in\calF, (\bs{x}_i,y_i)\in\calS] \subset [0, C]^N$: prediction evaluated over the training set $\calS$
    \item $\calG\bullet\calS \defeq \set{(\calL(y_1,f_{\bth}(\bs{x}_1)), \ldots, \calL(y_N,f_{\bth}(\bs{x}_N)))}[f_{\bth} \in \calF, (\bs{x}_i,y_i)\in\calS] \subset [0, LC]^N$: loss function evaluated over the training set $\calS$
\end{itemize}
